...
it('should scrape icon if size is not available', async () => {
  const res = await scrape(
    'http://localhost:8080/iconNoSize.html',
    browser,
    scrapeIcon,
  );
  expect(res).toMatchSnapshot();
...
it('should fallback to apple-touch-icon when icon is not available', async () => {
  const res = await scrape(
    'http://localhost:8080/appleTouchIcon.html',
    browser,
    scrapeIcon,
  );
  expect(res).toMatchSnapshot();
...
it('should fallback to msapplication-TileImage', async () => {
  const res = await scrape(
    'http://localhost:8080/tileImage.html',
    browser,
    scrapeIcon,
  );
  expect(res).toMatchSnapshot();
...
it('should fallback to shortcut icon', async () => {
  const res = await scrape(
    'http://localhost:8080/shortcutImage.html',
    browser,
    scrapeIcon,
  );
  expect(res).toMatchSnapshot();
...
it('should return absolute url', async () => {
  const res = await scrape(
    'http://localhost:8080/relIcon.html',
    browser,
    scrapeIcon,
  );
  expect(res).toMatchSnapshot();
